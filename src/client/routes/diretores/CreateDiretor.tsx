...
import { createDiretor } from "../../utils/diretores/createDiretor";
import toast from "react-simple-toasts";
import { Toast } from "../../components/Toast";
import { LinkButton } from "../../components/LinkButton";

const initialCreateDiretor = {
  nome: "",
  sobrenome: "",
  area: "",
  data_nasc: new Date(),
...
export function CreateDiretor() {
  const navigate = useNavigate();
  const isLoading = useGlobalStore((state) => state.isLoading);
  const setIsLoading = useGlobalStore((state) => state.setIsLoading);
  const [form, setForm] = useState(initialCreateDiretor);

  async function onSubmit(event: React.FormEvent) {
    event.preventDefault();
    setIsLoading(true);
    await createDiretor(form)
      .then(() => {
        toast("O diretor foi adicionado com sucesso", {
          render: (message) => (
            <Toast
      ...
      .catch(() => {
        toast("Não foi possível adicionar o diretor", {
          render: (message) => (
            <Toast
  ...
  return (
    <div className="bg-slate-800 h-[850px]">
      <div className="w-full text-center pt-10">
        <LinkButton to={`/diretores/`} className="text-xl text-white underline">
