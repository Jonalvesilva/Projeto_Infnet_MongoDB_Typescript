...
import { updateProfessor } from "../../utils/professores/updateProfessor";
import toast from "react-simple-toasts";
import { Toast } from "../../components/Toast";
import { LinkButton } from "../../components/LinkButton";

const initialEditProfessor = {
  nome: "",
  sobrenome: "",
  area: "",
  data_nasc: new Date(),
...
export function EditProfessor() {
  ...
  const [form, setForm] = useState(initialEditProfessor);

  async function onSubmit(event: React.FormEvent) {
    event.preventDefault();
    setIsLoading(true);
    await updateProfessor(form, params.id!)
      .then(() => {
        toast("O professor foi editado com sucesso", {
          render: (message) => (
            <Toast
        ...
        navigate(`/professores/${params.id}`);
      })
      .catch(() => {
        toast("Não foi possível editar o professor", {
          render: (message) => (
            <Toast
  ...
  return (
    <div className="bg-slate-800 h-[850px]">
      <div className="w-full text-center pt-10">
        <LinkButton
          to={`/professores/${params.id}`}
          className="text-xl text-white underline"
        >
          Voltar Para Perfil Professor
        </LinkButton>
      </div>
